...
pub(crate) fn generate_signed_spec_qr(
    ...
    generate_signed_qr(&content.to_sign(), &path, Msg::AddSpecs)?;
    Ok(path)
}
...
pub(crate) fn generate_signed_metadata_qr(
    ...
    info!("⚙️  Generating {}...", file_name);
    generate_signed_qr(&content.to_sign(), &path, Msg::LoadMetadata)?;
    Ok(path)
}

fn generate_signed_qr<P>(content: &[u8], target_path: P, msg_type: Msg) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
