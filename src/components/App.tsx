...
export default function App() {
    ...
    const routes = chainsName.map(name => <Route key={name} path={name} element={<Page currentName={name} allChains={allChains}/>} />)
  return (
      <HashRouter>
        <Routes>
            <Route path="/" element={<Page currentName={chainsName[0]} allChains={allChains}/>} />
            {routes}
        </Routes>
