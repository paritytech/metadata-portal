...
export default function Extension(chainSpec: ChainSpec) {
  const [selected, setSelected] = useState<InjectedExtension | undefined>(undefined)
...
async function extensionsToUpdate(chainSpec: ChainSpec): Promise<InjectedExtension[]> {
  const allInjected = await web3Enable('Metadata Portal')

  const extensions = await Promise.all(allInjected.map(async injected => {
  ...
  return extensions.filter(extension => {
    const current = extension.metadata.find(({genesisHash}) => genesisHash === chainSpec.genesisHash)
    if (!current) {
      return true
    }
