import {ChainSpec, QrInfo, RpcSource, SourceType, WasmSource} from "../scheme";
...
export default function MetadataTab({specs}: MetadataTabProps) {
  const {color, metadataQr, metadataVersion, nextMetadataVersion, nextMetadataQr } = specs;

  const qrs: LabeledQr[] = [{qr:metadataQr, label: `Current: #${metadataVersion}`}]
  ...
  return (
    <div className="w-fit">
      ...
      <div className="md:w-[468px] md:h-[468px]">
        <img src={process.env.PUBLIC_URL + qrs[selectedIdx].qr.path} alt="metadata qr code" />
      </div>
      <div className="px-4">
...
function SignedBy({signer}: SignedByProps) {
  return signer ?
      <span className="p-1 mx-2 rounded-full text-xs text-green-700 bg-green-100">Signed by {signer}</span> :
      <span className="p-1 mx-2 rounded-full text-red-700 bg-red-100">Unsigned</span>
}
